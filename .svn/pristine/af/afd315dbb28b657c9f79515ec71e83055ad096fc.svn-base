	...
	<nav class="navbar navbar-inverse custom-navbar" style="margin:0 -10px;">
			<div class="container">
				<div class="navbar-header">
					...
					<a href="mobile_lpt_info" class="btn btn-xs btn-success pull-right" style="margin:8px 15px 0px; font-size:14px;"><i class="fa fa-user-md"></i>&nbsp;医生信息</a>
				</div>
			</div>
		</nav>
		<!--轮播图start-->
    	...
    	<c:forEach var="item" items="${Pie_keshi}" varStatus="status">
    	keshi_legend_list.push('${item.serialName}');
    	</c:forEach> 
    	var keshi_data_list = [];
		<c:forEach var="item" items="${Pie_keshi}" varStatus="status">
		var keshi_dict= {};
		keshi_dict["name"]='${item.serialName}';
		keshi_dict["value"]=${item.serialCount1};
		...
		var course_dict= {};
		course_legend_list.push('${item.serialName}');
		course_dict["name"]='${item.serialName}';
		course_dict["value"]=${item.serialCount1};
		...
		date_legend_list.push('${item.serialName}');
		
		date_data1_list.push(parseInt('${item.serialCount1}'));
		date_data2_list.push(parseInt('${item.serialCount2}'));
		</c:forEach>
		...
		setInterval(function() {
			...
			if(end >100){
				date_line_option.dataZoom[0].end = 60;
				date_line_option.dataZoom[0].start =0;
				end = 60;
				start = 0;
			}else{
				date_line_option.dataZoom[0].start=start;
				date_line_option.dataZoom[0].end=end;	
			}
			Date_info_chart.setOption(date_line_option, true);
		}, 1500);
		...
		$("#city-name").text(selectedProvince);
		 $.ajax({
    		 ...
    		 success:function(data){
    			 var obj = eval('(' + data + ')');
    			 for(i in obj){
    				 htm += "<tr><td>"+obj[i].serialName+"</td><td>"+obj[i].serialCount1+"</td></tr>";
    			 }
